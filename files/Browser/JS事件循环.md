...
首先，先记住一句话**单线程是异步产生的原因，事件循环是异步的实现方式**，读懂本篇文章后，再回过头想想这句话
...
- 在 JS 执行期间，渲染主线程处于「阻塞」状态，无法执行任何任务
- 因为 JS 可能会改变 DOM，所以渲染主线程必须停下所有工作，等待 JS 执行完毕后，再继续渲染
- 就算你写的是死循环，主线程依旧会继续等待，因为他不知道这个是死循环，他觉得这个非常重要，一定要先执行
